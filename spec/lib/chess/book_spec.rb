...
RSpec.describe Chess::Book do
  ...
  describe '#commit_move_intention' do
    describe 'when the piece choosed to move, is by the enemy team' do
      it 'returns an error of the submit' do
        expect(book.move_intention('P', 'e7', 'e5', Chess::WHITE_TEAM)).to be(Chess::ERR_CELL_OCCUPED_BY_ENEMY)
      end
    end

    describe 'when the origin cell is empty' do
      it 'returns an error of the submit' do
        expect(book.move_intention('P', 'e3', 'e4', Chess::WHITE_TEAM)).to be(Chess::ERR_EMPTY_ORIGIN_CELL)
      end
    end

    describe 'when the origin cell is occuped by another cell' do
      it 'returns an error of the submit' do
        expect(book.move_intention('N', 'e2', 'f4', Chess::WHITE_TEAM)).to be(Chess::ERR_WRONG_PIECE_AT_CELL)
      end
    end

    describe "when selected piece can't be moved to target cell" do
      it 'returns an error of the commit' do
        expect(book.move_intention('P', 'e2', 'f3', Chess::WHITE_TEAM)).to be(Chess::ERR_CANT_REACH_TARGET_CELL)
      end
    end

    describe 'when the move can be commited' do
      it 'returns succes' do
        expect(book.move_intention('P', 'e2', 'e3', Chess::WHITE_TEAM)).to be(Chess::COMMIT_SUCCESS)
      end
    end
    ...
    describe 'when move a piece, expose the king to death' do
      before do
        puts board
        book.move_intention('P', 'a2', 'a4', Chess::WHITE_TEAM)
        book.move_intention('P', 'e7', 'e6', Chess::BLACK_TEAM)
        book.move_intention('N', 'g1', 'h3', Chess::WHITE_TEAM)
        book.move_intention('Q', 'd8', 'h4', Chess::BLACK_TEAM)
        puts board
      end

      it 'rollbacks the commitment' do
        expect(book.move_intention('P', 'f2', 'f4', Chess::WHITE_TEAM)).to be(Chess::ROLLBACK_SUCCES)
      end
    end
  end
  ...
  describe '#castle_intention_on' do
    describe 'when castling can be done' do
      before do
        puts board
        book.move_intention('N', 'g1', 'h3', Chess::WHITE_TEAM)
        book.move_intention('P', 'e2', 'e3', Chess::WHITE_TEAM)
        book.move_intention('B', 'f1', 'e2', Chess::WHITE_TEAM)
        puts board
      end
    ...
    describe "when castling can't be done because path is not empty" do
      ...
      it 'returns error commit' do
        expect(book.castle_intention_on(Chess::KING_SIDE, Chess::WHITE_TEAM)).to be(Chess::ERR_CANT_CASTLING)
      end
    end
    ...
    describe "when castling can't be done because is not rook's first move" do
      before do
        ...
        book.move_intention('P', 'e2', 'e3', Chess::WHITE_TEAM)
        book.move_intention('B', 'f1', 'e2', Chess::WHITE_TEAM)
        book.move_intention('R', 'h1', 'g1', Chess::WHITE_TEAM)
        book.move_intention('R', 'g1', 'h1', Chess::WHITE_TEAM)
        puts board
      end
    ...
    describe "when castling can't be done because king will be death" do
      before do
        puts board
        book.move_intention('N', 'b1', 'a3', Chess::WHITE_TEAM)
        book.move_intention('P', 'd2', 'd3', Chess::WHITE_TEAM)
        book.move_intention('B', 'c1', 'e3', Chess::WHITE_TEAM)
        book.move_intention('Q', 'd1', 'd2', Chess::WHITE_TEAM)
        book.move_intention('P', 'b2', 'b3', Chess::WHITE_TEAM)
        book.move_intention('P', 'e7', 'e6', Chess::BLACK_TEAM)
        book.move_intention('B', 'f8', 'a3', Chess::BLACK_TEAM)
        puts board
      end
